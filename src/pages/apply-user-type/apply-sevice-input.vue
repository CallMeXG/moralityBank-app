<template>
  <div class="positionClass">
    <van-sticky><x-header class="headerBackgroundColor" >服务对象认证</x-header></van-sticky>

    <p style="margin: 10px;font-size: 14px;color: gray">您要认证的账户类型为: <span style="color: red">服务对象</span></p>
    ...
    methods: {
      ...
      onSubmit(values) {
        ...
        if (values.sex === ''){
          ...
          paramsData.append('confirmType', '3')
          ...
          this.seviceObjectApplySubmit(paramsData)
        }


      },
      seviceObjectApplySubmit(params) {
        Toast.loading({
      ...
      //获取省份列表
      gainProvinceList(){
        this.$api.AddrList.provinceList_interface({
        }).then(response => {
          if (response.status === 200){
            if (response.data.status === 'SUCCESS'){
