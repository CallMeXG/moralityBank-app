<template>
  <div class="positionClass">
    ...
    <p style="margin: 10px;font-size: 14px;color: gray">您要认证的账户类型为: <span style="color: red">个人志愿者</span></p>
    ...
    <van-form @submit="onSubmit">
      ...
      <van-field required name="uploader" label="证件照片正面">
        <van-uploader :before-read="beforeRead" v-model="uploader" :max-count="1" slot="input"/>
      </van-field>
      <van-field required name="uploader" label="证件照片反面">
  ...
  export default {
    ...
    data() {
      return {
        ...
        flagUpdate: false,//是否为修改信息页面，复用
        updateMsg: new Object()
      }
    },

    mounted() {
      let queryObj = this.$route.query
      if (queryObj.updateMsg !== undefined) {
        this.flagUpdate = true
        this.updateMsg = queryObj.updateMsg

        this.name = this.updateMsg.name
        this.sex = this.updateMsg.sex
        this.age = this.updateMsg.age
        this.credentialNum = this.updateMsg.credentialNum
        this.uploader = [{
          url: this.updateMsg.photoUrlUp
        }]
        this.uploader_down = [{
          url: this.updateMsg.photoUrlDn
        }]
        this.strarea = this.columnsArea[0].text
        this.gainStreetList(this.strAreaCode)
        this.strstreet = this.updateMsg.bsStreet.streetName
        this.strStreetCode = '220105001'
        this.gainCommunityList(this.strStreetCode)
        this.strcommunity = this.updateMsg.bsCommunity.communityName
        this.straddr = this.updateMsg.address
        if (this.updateMsg.notice !== undefined) {
          this.introduction = this.updateMsg.notice
        }
        if (this.updateMsg.otherPhotoArr !== undefined){
          for (let i = 0; i < this.updateMsg.otherPhotoArr.length; i++) {
            this.other_uploader.push({
              url: this.updateMsg.otherPhotoArr[i]
            })
          }
        }
    ...
    methods: {
      ...
      onSubmit(values) {
        ...
        if (values.sex === '') {
          ...
          paramsData.append('confirmType', '1')
          ...
          if (this.uploader[0].file !== undefined){
            paramsData.append('photoUp', this.uploader[0].file)
          }
          if (this.uploader_down[0].file !== undefined){
          ...
          if (this.flagUpdate) {
            paramsData.append('id', this.updateMsg.id)
          }
          this.userApplySubmit(paramsData)

        }

      },
      //用户认证
      userApplySubmit(params) {
        ...
        this.$api.UserApply.userApplySubmit_post(params).then(response => {
          Toast.clear()
